...
const WhatsAppIntegration: React.FC<WhatsAppIntegrationProps> = ({
  ...
  const [businessEtiquetteEnabled, setBusinessEtiquetteEnabled] = useState(
    enableBusinessEtiquette
  )
  ...
  const processTemplate = useCallback(
    (template: WhatsAppTemplate, contact: WhatsAppContact) => {
      ...
      if (businessEntity) {
        ...
        if (businessEntity.amount) {
          processedMessage = processedMessage.replace(
            /\{\{amount\}\}/g,
            formatIDR(businessEntity.amount)
          )

          // Materai calculation
          if (businessEntity.amount >= 5000000) {
            const materaiAmount =
              businessEntity.amount >= 1000000000 ? 20000 : 10000
            processedMessage = processedMessage.replace(
              /\{\{materai_amount\}\}/g,
              formatIDR(materaiAmount)
            )
            processedMessage = processedMessage.replace(
              /\{\{materai_info\}\}/g,
  ...
  const handleSendMessage = useCallback(
    (messageText: string, template?: WhatsAppTemplate) => {
      if (!selectedContact) {
                    ...
                    <Alert
                      ...
                      description={
                        <div>
                          ...
                          <br />
                          <Text type='secondary'>
                            {businessEntity.type} • {businessEntity.id}
                          </Text>
                          {businessEntity.amount && (
                            <>
                              <br />
                              <Text>
                                Nilai: {formatIDR(businessEntity.amount)}
                              </Text>
                            </>
                          )}
                        </div>
                    ...
                    <Select
                      ...
                      onChange={value => {
                        ...
                        if (template && selectedContact) {
                          setCustomMessage(
                            processTemplate(template, selectedContact)
                          )
                        }
                      }}
                    >
              ...
              renderItem={msg => (
                <List.Item>
                  <List.Item.Meta
                    ...
                    title={
                      <Space>
                        {contacts.find(c => c.id === msg.contactId)?.name ||
                          'Unknown'}
                        <Tag
                          color={
                            msg.status === 'read'
                        ...
                        {msg.businessContext && (
                          <Tag color='purple'>
                            {msg.businessContext.entityType}{' '}
                            {msg.businessContext.entityId}
                          </Tag>
                        )}
                      </Space>
                    }
