...
export const OfflineStatus: React.FC<{ showDetails?: boolean }> = ({ showDetails = false }) => {
      ...
      <Alert
        message={
          <Space>
            ...
            {state.queueCount > 0 && (
              <Badge count={state.queueCount} style={{ backgroundColor: '#fa8c16' }} />
            )}
          </Space>
        }
        type={state.isOnline ? (state.queueCount > 0 ? 'warning' : 'success') : 'error'}
        action={
          <Space>
            {showDetails && (
        ...
        <Row gutter={16}>
          <Col span={12}>
            <Statistic
              ...
              prefix={state.isOnline ? <WifiOutlined /> : <DisconnectOutlined />}
              ...
              <List.Item>
                <List.Item.Meta
                  avatar={
                    ...
                    item.status === 'failed' ? <WarningOutlined style={{ color: '#f5222d' }} /> :
