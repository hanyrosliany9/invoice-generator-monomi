...
const MobileOptimizedLayout: React.FC<MobileOptimizedLayoutProps> = ({
  ...
  const MobileHeader = () => (
    <Header
      style={{
        ...
        transform: adaptiveHeader && isMobile ? 
          `translateY(${headerVisible ? '0' : '-100%'})` : 'none',
  ...
  const BottomNavigation = () => (
    <Affix offsetBottom={0}>
      ...
      >
        {allNavigationItems
          .filter(item => item.quickAction || item.indonesianPriority === 'high')
          .slice(0, 5)
          .map(item => (
            <Button
              key={item.key}
    ...
    <Layout style={{ minHeight: '100vh' }}>
      ...
      {showFloatingActions && (
        <FloatButton.Group
          trigger="click"
          type="primary"
          style={{ right: 24, bottom: showBottomNavigation && isMobile ? 80 : 24 }}
          icon={<PlusOutlined />}
      ...
      {!isMobile && (
        <Footer style={{ textAlign: 'center', background: '#f0f2f5' }}>
          <Text type="secondary">
            Monomi Business Management System ©2025 - Dioptimalkan untuk Bisnis Indonesia
          </Text>
        </Footer>
      )}
    </Layout>
