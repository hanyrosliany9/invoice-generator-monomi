...
import { Table, Card, Space, Button, Typography, Tag, Tooltip, Input, Select, DatePicker, Row, Col } from 'antd'
...
export interface SmartTableProps<T = BusinessEntity> extends Omit<TableProps<T>, 'columns'> {
  // Data configuration
...
const SmartTable = <T extends BusinessEntity>({
  ...
  const processedData = useMemo(() => {
    return measurePerformance('dataProcessing', () => {
      ...
      if (searchText && enableGlobalSearch) {
        ...
        const fieldsToSearch = searchableFields || ['title', 'number', 'client.name', 'client.company']
  ...
  }, [data, searchText, activeFilters, sortConfig, enableGlobalSearch, searchableFields, measurePerformance])
            ...
            {customActions.map(action => (
              <Tooltip key={action.key} title={action.label}>
                <Button 
                  ...
                  style={{ display: action.visible?.(record) === false ? 'none' : 'inline-block' }}
  ...
  }, [entityType, hiddenColumns, enableMateraiIndicator, customActions, onRowAction])
  ...
  const handleExport = useCallback((format: string) => {
    recordMetric('export_initiated', { format, recordCount: processedData.length })
  ...
  const businessMetrics = useMemo(() => {
    ...
    return {
      ...
      highValueCount: processedData.filter(item => item.amount >= 5000000).length
    }
