...
export const BreadcrumbProvider: React.FC<BreadcrumbProviderProps> = ({
  ...
  const getEntityDisplayName = useCallback(
    (entityType: string, entityData: any): string => {
      switch (entityType) {
        case 'client':
          return entityData.name || entityData.company || 'Unknown Client'
        case 'project':
          return (
            entityData.number || entityData.description || 'Unknown Project'
          )
        case 'quotation':
          return (
            entityData.quotationNumber ||
            `QT-${entityData.id?.slice(-6)}` ||
            'Unknown Quotation'
          )
        case 'invoice':
          return (
            entityData.invoiceNumber ||
            `INV-${entityData.id?.slice(-6)}` ||
            'Unknown Invoice'
          )
        default:
  ...
  const buildBreadcrumb = useCallback(
    (entityType: string, entityData: any): BreadcrumbItem[] => {
      ...
      switch (entityType) {
          ...
          if (entityData.quotation?.project?.client) {
            items.push(
              createBreadcrumbItem(
                'client',
                entityData.quotation.project.client
              )
            )
          }
          if (entityData.quotation?.project) {
            items.push(
              createBreadcrumbItem('project', entityData.quotation.project)
            )
          }
        ...
        case 'quotation':
          ...
          if (entityData.project?.client) {
            items.push(
              createBreadcrumbItem('client', entityData.project.client)
            )
          }
