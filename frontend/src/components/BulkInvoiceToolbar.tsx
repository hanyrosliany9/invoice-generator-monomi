        ...
        <div className='flex justify-between items-center'>
          <Space size='small'>
            <Tooltip
              title={
                canBulkSend()
                  ? `Kirim ${statusCounts.DRAFT} invoice draft`
                  : 'Tidak ada invoice draft yang dipilih'
              }
            >
              <Button
                size='small'
            ...
            <Tooltip
              title={
                canBulkMarkPaid()
                  ? `Tandai ${statusCounts.SENT + statusCounts.OVERDUE} invoice sebagai lunas`
                  : 'Tidak ada invoice terkirim yang dipilih'
              }
            >
              <Button
                size='small'
              ...
              >
                Tandai Lunas{' '}
                {statusCounts.SENT + statusCounts.OVERDUE > 0 &&
                  `(${statusCounts.SENT + statusCounts.OVERDUE})`}
              </Button>
            </Tooltip>
          ...
          <Space size='small'>
            <Tooltip
              title={
                canBulkDelete()
                  ? 'Hapus invoice yang dipilih'
                  : 'Hanya invoice draft dan dibatalkan yang dapat dihapus'
              }
            >
              <Button
                size='small'
              ...
              >
                Hapus{' '}
                {statusCounts.DRAFT + statusCounts.CANCELLED > 0 &&
                  `(${statusCounts.DRAFT + statusCounts.CANCELLED})`}
              </Button>
            </Tooltip>
          </Space>
        </div>
