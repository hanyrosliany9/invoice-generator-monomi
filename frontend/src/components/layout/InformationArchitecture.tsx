...
const InformationArchitecture: React.FC<InformationArchitectureProps> = ({
  ...
  const breadcrumbItems = useMemo(() => {
    ...
    const findItemPath = (items: NavigationItem[], targetPath: string, currentPath: string[] = []): string[] | null => {
  ...
  const renderNavigationMenu = useCallback((items: NavigationItem[]) => {
  ...
  const businessContextPanel = (
    <Space direction="vertical" style={{ width: '100%' }} size="middle">
          ...
          <Space direction="vertical" style={{ width: '100%' }}>
            <Space>
              ...
              <div>
                ...
                <br />
                <Text type="secondary">
                  {businessContext.currentEntity.type} • {businessContext.currentEntity.status}
                </Text>
              </div>
            </Space>
      ...
      {/* Related entities */}
      {businessContext?.relatedEntities && businessContext.relatedEntities.length > 0 && (
        <Card size="small" title="Entitas Terkait">
          {businessContext.relatedEntities.map((group, index) => (
            <div key={index} style={{ marginBottom: 12 }}>
              <Text strong style={{ fontSize: '12px' }}>{group.type}:</Text>
      ...
      {/* Notifications */}
      {businessContext?.notifications && businessContext.notifications.length > 0 && (
                ...
                <div
                  ...
                  style={{ 
                    ...
                    backgroundColor: notification.type === 'error' ? '#fff2f0' : 
                                   notification.type === 'warning' ? '#fffbe6' : '#f6ffed'
                  }}
                >
                  <Space>
                    {notification.type === 'error' && <WarningOutlined style={{ color: 'red' }} />}
                    {notification.type === 'warning' && <WarningOutlined style={{ color: 'orange' }} />}
                    {notification.type === 'success' && <CheckCircleOutlined style={{ color: 'green' }} />}
                    <div>
                      <Text strong style={{ fontSize: '12px' }}>{notification.title}</Text>
                      <br />
                      <Text type="secondary" style={{ fontSize: '11px' }}>
        ...
        <div style={{ 
          ...
          {sidebarCollapsed ? (
            ...
            <Space>
              ...
              <div>
                <Title level={5} style={{ margin: 0 }}>Monomi</Title>
            ...
            <Space>
              ...
              {showNotifications && (
                <Tooltip title="Notifikasi">
                  <Badge count={businessInsights?.unreadNotificationsCount || 0}>
                    <Button
                      type="text"
                      icon={<BellOutlined />}
