...
export interface SecurityVulnerability {
  id: string
  type: 'xss' | 'sql_injection' | 'csrf' | 'sensitive_data' | 'weak_auth' | 'materai_compliance' | 'indonesian_privacy'
...
export interface SecurityRule {
  id: string
  name: string
  category: 'input_validation' | 'authentication' | 'authorization' | 'data_protection' | 'indonesian_compliance'
  description: string
  severity: 'critical' | 'high' | 'medium' | 'low'
  enabled: boolean
...
const SecurityAudit: React.FC<SecurityAuditProps> = ({
  ...
  const indonesianSecurityRules: SecurityRule[] = useMemo(() => [
    ...
    },
    {
      ...
      description: 'Mematuhi UU No. 27 Tahun 2022 tentang Perlindungan Data Pribadi',
      severity: 'critical',
    ...
    },
    {
      ...
      category: 'data_protection',
      description: 'Proteksi dokumen quotation dan invoice sesuai standar Indonesia',
      severity: 'high',
      enabled: true,
      indonesianSpecific: true
    },
  ...
  const performRuleCheck = async (rule: SecurityRule): Promise<SecurityVulnerability[]> => {
    const vulnerabilities: SecurityVulnerability[] = []
  ...
  const checkXSSVulnerabilities = async (): Promise<SecurityVulnerability[]> => {
    ...
    if (Math.random() > 0.7) { // 30% chance to find XSS issue
      vulnerabilities.push({
        id: `xss_${Date.now()}`,
        type: 'xss',
        severity: 'high',
        title: 'Potensi XSS pada Input Form',
        description: 'Input form tidak memiliki sanitasi yang memadai terhadap script injection',
        location: 'components/forms/QuotationForm.tsx:line 45',
        impact: 'Penyerang dapat menjalankan script berbahaya di browser pengguna',
        recommendation: 'Implementasikan sanitasi input menggunakan DOMPurify dan validasi server-side',
        cwe: 'CWE-79',
        cvss: 7.5,
        foundAt: new Date(),
        status: 'open',
  ...
  const checkMateraiCompliance = async (): Promise<SecurityVulnerability[]> => {
    ...
    if (Math.random() > 0.8) { // 20% chance to find materai issue
      vulnerabilities.push({
        id: `materai_${Date.now()}`,
        type: 'materai_compliance',
        severity: 'medium',
        title: 'Ketidakakuratan Kalkulasi Materai',
        description: 'Perhitungan materai tidak sesuai dengan UU No. 13 Tahun 1985',
        location: 'utils/materaiCalculator.ts',
        impact: 'Potensi pelanggaran regulasi Indonesia dan denda pajak',
        recommendation: 'Update algoritma materai sesuai dengan tarif terbaru (Rp 10.000 untuk transaksi 5-1M, Rp 20.000 untuk >1M)',
        foundAt: new Date(),
        status: 'open',
  ...
  const checkPrivacyCompliance = async (): Promise<SecurityVulnerability[]> => {
    ...
    if (Math.random() > 0.6) { // 40% chance to find privacy issue
      vulnerabilities.push({
        ...
        impact: 'Pelanggaran UU No. 27 Tahun 2022, potensi denda hingga 2% dari pendapatan tahunan',
        recommendation: 'Implementasikan enkripsi AES-256 untuk data sensitif seperti NIK, NPWP',
        foundAt: new Date(),
        status: 'open',
        affectsIndonesianCompliance: true,
        privacyImpact: true,
  ...
  const checkSensitiveDataExposure = async (): Promise<SecurityVulnerability[]> => {
    const vulnerabilities: SecurityVulnerability[] = []
    
    // Check for exposed sensitive Indonesian data
    if (Math.random() > 0.75) { // 25% chance to find data exposure
      vulnerabilities.push({
        ...
        recommendation: 'Masking data sensitif dalam log dan API response, implementasikan field-level encryption',
        cwe: 'CWE-200',
        cvss: 8.5,
        foundAt: new Date(),
        status: 'open',
  ...
  const checkCSRFProtection = async (): Promise<SecurityVulnerability[]> => {
    ...
    if (Math.random() > 0.85) { // 15% chance to find CSRF issue
      vulnerabilities.push({
        id: `csrf_${Date.now()}`,
        type: 'csrf',
        severity: 'high',
  ...
  const checkBusinessDocumentSecurity = async (): Promise<SecurityVulnerability[]> => {
    const vulnerabilities: SecurityVulnerability[] = []
    
    // Check business document security
    if (Math.random() > 0.7) { // 30% chance to find document security issue
      vulnerabilities.push({
        id: `doc_security_${Date.now()}`,
        type: 'weak_auth',
        severity: 'medium',
        title: 'Keamanan Dokumen Bisnis Lemah',
        description: 'Dokumen quotation dan invoice dapat diakses tanpa otorisasi proper',
        location: 'api/documents/download',
        impact: 'Kebocoran informasi bisnis sensitif',
        recommendation: 'Implementasikan access control berbasis role dan audit trail',
        foundAt: new Date(),
        status: 'open',
        affectsIndonesianCompliance: true,
        businessCritical: true
  ...
  // Calculate security scores
  const calculateSecurityScores = (vulnerabilities: SecurityVulnerability[]) => {
    ...
    const overall = Math.max(0, 100 - (criticalIssues * 25 + highIssues * 15 + mediumIssues * 10))
    
    // Indonesian compliance scores
    const indonesianIssues = vulnerabilities.filter(v => v.affectsIndonesianCompliance)
    ...
    const authIssues = vulnerabilities.filter(v => v.type === 'weak_auth' || v.type === 'csrf')
      ...
      {isScanning && (
        <Card style={{ marginBottom: 24 }}>
          <Space direction="vertical" style={{ width: '100%' }}>
            ...
            <Text type="secondary">
              Memeriksa {securityRules.filter(rule => rule.enabled).length} aturan keamanan...
            </Text>
          </Space>
        </Card>
      )}
          ...
          <Col span={4}>
            <Card>
              <Statistic
                ...
                suffix="/100"
                valueStyle={{ color: getScoreColor(currentScan.indonesianCompliance) }}
          ...
          <Col span={4}>
            <Card>
              <Statistic
                ...
                suffix="/100"
                valueStyle={{ color: getScoreColor(currentScan.materaiCompliance) }}
          ...
          <Col span={4}>
            <Card>
              <Statistic
                ...
                suffix="/100"
                valueStyle={{ color: getScoreColor(currentScan.privacyCompliance) }}
          ...
          <Col span={4}>
            <Card>
              <Statistic
                ...
                suffix="/100"
                valueStyle={{ color: getScoreColor(currentScan.authenticationSecurity) }}
                prefix={<LockOutlined />}
              />
            </Card>
          </Col>
          <Col span={4}>
            <Card>
              <Statistic
                ...
                suffix="/100"
                valueStyle={{ color: getScoreColor(currentScan.dataProtection) }}
                prefix={<FileProtectOutlined />}
              />
            </Card>
          </Col>
          ...
          <Card title="Riwayat Scan">
            <Timeline>
              {scanHistory.map((scan) => (
                <Timeline.Item
                  key={scan.id}
                  color={scan.score >= 80 ? 'green' : scan.score >= 60 ? 'orange' : 'red'}
                  dot={
                    scan.status === 'completed' ? <CheckCircleOutlined /> :
                    scan.status === 'failed' ? <WarningOutlined /> :
                    <SecurityScanOutlined />
                  }
                >
                  <div>
        ...
        {selectedVulnerability && (
          <Space direction="vertical" style={{ width: '100%' }} size="large">
            <Alert
              ...
              type={selectedVulnerability.severity === 'critical' ? 'error' : 'warning'}
              showIcon
            />
            ...
            <Row gutter={16}>
              <Col span={12}>
                <Card size="small" title="Basic Information">
                  <p><strong>Type:</strong> {selectedVulnerability.type}</p>
                  ...
                  {selectedVulnerability.cvss && (
                    <p><strong>CVSS Score:</strong> {selectedVulnerability.cvss}</p>
                  )}
                </Card>
              </Col>
              <Col span={12}>
                <Card size="small" title="Indonesian Business Impact">
                  <p><strong>Affects Indonesian Compliance:</strong> 
                    {selectedVulnerability.affectsIndonesianCompliance ? ' Ya' : ' Tidak'}
                  </p>
                  <p><strong>Materai Related:</strong> 
                    {selectedVulnerability.materaiRelated ? ' Ya' : ' Tidak'}
                  </p>
                  <p><strong>Privacy Impact:</strong> 
                    {selectedVulnerability.privacyImpact ? ' Ya' : ' Tidak'}
                  </p>
                  <p><strong>Business Critical:</strong> 
