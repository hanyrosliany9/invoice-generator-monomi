...
export const InvoiceTableRowSkeleton: React.FC<{ rows?: number }> = ({ rows = 5 }) => (
  <div className="space-y-3">
    {Array.from({ length: rows }).map((_, index) => (
      <div key={index} className="flex items-center space-x-4 p-4 border rounded-lg">
        {/* Checkbox */}
        <Skeleton.Button active size="small" shape="square" style={{ width: 16, height: 16 }} />
...
export const RevenueCardSkeleton: React.FC = () => (
  ...
  >
    <div className="flex items-center space-x-4 mb-4">
      <Skeleton.Avatar active size={56} shape="square" style={{ backgroundColor: 'rgba(255, 255, 255, 0.3)' }} />
      <div className="flex-1 space-y-2">
        <Skeleton.Input active size="small" style={{ width: 100, backgroundColor: 'rgba(255, 255, 255, 0.4)' }} />
        <Skeleton.Input active size="large" style={{ width: 120, backgroundColor: 'rgba(255, 255, 255, 0.4)' }} />
      </div>
    </div>
    <Skeleton.Button active style={{ width: '100%', height: 8, backgroundColor: 'rgba(255, 255, 255, 0.3)' }} />
  </Card>
);

// Form Field Skeleton - matches form inputs
export const FormFieldSkeleton: React.FC<{ rows?: number }> = ({ rows = 6 }) => (
    ...
    <div className="flex justify-between items-center">
      ...
      <Space>
        {Array.from({ length: 5 }).map((_, index) => (
          <Skeleton.Button key={index} active size="small" style={{ width: 80 }} />
        ))}
      </Space>
    </div>
  </Card>
...
export const InvoicePageSkeleton: React.FC = () => (
  <div>
    {/* Page Header */}
    <div className="mb-6">
      <Skeleton.Input active size="large" style={{ width: 200, height: 40 }} className="mb-6" />
...
export const ModalContentSkeleton: React.FC<{ type?: 'form' | 'detail' }> = ({ type = 'form' }) => {
  if (type === 'detail') {
    return (
      <div className="space-y-6">
        <BreadcrumbSkeleton />
