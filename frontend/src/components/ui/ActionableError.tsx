...
import { Alert, Button, Space, Typography, Card, Steps, Collapse } from 'antd';
import { 
  ReloadOutlined, 
  SettingOutlined, 
  PhoneOutlined, 
...
const getErrorClassification = (errorMessage: string, context: string = 'general') => {
  ...
  if (message.includes('materai') || message.includes('npwp') || message.includes('pajak')) {
    return {
      type: 'business',
      severity: 'medium',
      ...
      solutions: [
        {
          ...
          description: 'Invoice hanya bisa dibuat dari quotation yang disetujui',
          steps: [
...
export const ActionableError: React.FC<ActionableErrorProps> = ({
  ...
  const handleContactSupport = () => {
    if (onContactSupport) {
      ...
      window.open('mailto:support@monomi.id?subject=Error Report&body=' + encodeURIComponent(
    ...
    <div className={className}>
            ...
            label: (
              <div className="flex items-center space-x-2">
                <CheckCircleOutlined className={
                  solution.priority === 'high' ? 'text-red-500' :
                  solution.priority === 'medium' ? 'text-orange-500' : 'text-blue-500'
                } />
                <Text strong>{solution.title}</Text>
                <Text type="secondary" className="text-xs">
                  ({solution.priority === 'high' ? 'Prioritas Tinggi' : 
                    solution.priority === 'medium' ? 'Prioritas Sedang' : 'Prioritas Rendah'})
                </Text>
              </div>
            ),
            children: (
      ...
      <Card className="mt-4" size="small">
        <div className="space-y-2">
          <Text strong className="text-sm">💡 Tips Cepat:</Text>
          <ul className="text-sm space-y-1 ml-4">
            ...
            {context === 'invoice' && (
              <li>Untuk invoice, pastikan klien dan proyek sudah dipilih dengan benar</li>
            )}
            {context === 'quotation' && (
              <li>Quotation harus disetujui sebelum bisa dibuat menjadi invoice</li>
            )}
          </ul>
        </div>
      </Card>
...
export const QuickErrorToast = {
  ...
  validation: () => "📝 Periksa data yang dimasukkan. Pastikan semua field terisi dengan benar.",
  permission: () => "🚫 Akses ditolak. Hubungi administrator untuk mendapatkan izin.",
  business: () => "📋 Periksa aturan bisnis. Pastikan data sesuai ketentuan Indonesia.",
