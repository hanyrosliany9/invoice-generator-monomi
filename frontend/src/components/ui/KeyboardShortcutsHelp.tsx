...
import { Modal, Card, Typography, Row, Col, Tag, Divider, Space, Tabs, Badge } from 'antd';
import { 
  KeyOutlined,
  RocketOutlined,
  CompassOutlined,
...
const ShortcutItem: React.FC<ShortcutItemProps> = ({ shortcut, highlight = false }) => (
...
const getCategoryIcon = (category: string) => {
  switch (category) {
    case 'navigation': return <CompassOutlined />;
    case 'actions': return <ThunderboltOutlined />;
    case 'editing': return <EditOutlined />;
...
const getCategoryColor = (category: string) => {
  switch (category) {
    case 'navigation': return '#1890ff';
    case 'actions': return '#52c41a';
    case 'editing': return '#fa8c16';
    case 'general': return '#722ed1';
    default: return '#666';
  }
};
  ...
  const categoryTabs = [
    {
      ...
      label: (
        <Space>
          <KeyOutlined />
          All Shortcuts
          <Badge count={shortcuts.length} style={{ backgroundColor: '#f0f0f0', color: '#666' }} />
        </Space>
      ),
      children: (
            ...
            <div>
              ...
              <div className="space-y-2">
                {contextShortcuts.map((shortcut, index) => (
                  <ShortcutItem key={index} shortcut={shortcut} highlight={true} />
                ))}
              </div>
              <Divider />
            </div>
    ...
    },
    {
      ...
      label: (
        <Space>
          <CompassOutlined />
          Navigation
          <Badge count={groupedShortcuts.navigation?.length || 0} style={{ backgroundColor: '#1890ff' }} />
        </Space>
      ),
      children: (
        <div className="space-y-2">
    ...
    },
    {
      ...
      label: (
        <Space>
          <ThunderboltOutlined />
          Actions
          <Badge count={groupedShortcuts.actions?.length || 0} style={{ backgroundColor: '#52c41a' }} />
        </Space>
      ),
      children: (
        <div className="space-y-2">
              ...
              <Card size="small">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div><Text type="secondary" className="text-sm">Toggle materai status on invoices</Text></div>
                  </div>
                  <Tag color="orange" className="font-mono">Alt + M</Tag>
                </div>
              </Card>
              <Card size="small">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div><Text type="secondary" className="text-sm">Calculate 11% PPN automatically</Text></div>
                  </div>
                  <Tag color="orange" className="font-mono">Alt + P</Tag>
                </div>
              </Card>
              <Card size="small">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div><Text type="secondary" className="text-sm">Show bulk action panel</Text></div>
                  </div>
                  <Tag color="orange" className="font-mono">Ctrl + Shift + B</Tag>
                </div>
              </Card>
            </div>
          </div>
    ...
    >
      <div className="mb-4">
        <Paragraph className="text-sm text-gray-600">
          Master these shortcuts to boost your productivity by <strong>15%</strong> and save <strong>2 minutes per invoice</strong>. 
          Press <Tag className="font-mono mx-1">F1</Tag> anytime to open this help.
        </Paragraph>
      </div>
      ...
      <div className="text-center">
        <Text type="secondary" className="text-xs">
          💡 Tip: These shortcuts work globally across the application unless you're typing in a form field
        </Text>
      </div>
    </Modal>
  );
