...
interface UserTestingContextType {
  ...
  startSession: (
    testType: UserTestingSession['testType'],
    component: string
  ) => void
  endSession: () => void
...
export const UserTestingProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  const [currentSession, setCurrentSession] =
    useState<UserTestingSession | null>(null)
  const [sessionHistory, setSessionHistory] = useState<UserTestingSession[]>([])

  // Start a new testing session
  const startSession = useCallback(
    (testType: UserTestingSession['testType'], component: string) => {
  ...
  const recordError = useCallback(
    (error: Omit<UserError, 'id' | 'timestamp'>) => {
  ...
  const updateMetrics = useCallback(
    (metrics: Partial<UserTestingMetrics>) => {
      if (!currentSession) return
  ...
  if (!currentSession) {
    return (
      <Card
        ...
        style={{ margin: '16px 0' }}
      >
        <Text>Framework untuk menguji pengalaman pengguna dengan target:</Text>
        <ul>
  ...
  return (
    <>
      <Card
        ...
        extra={
          <Space>
            <Button onClick={() => setIsVisible(true)}>Detail</Button>
      ...
      >
        <Row gutter={16}>
          ...
          <Col span={8}>
            <Statistic
              ...
              valueStyle={{
                color:
                  currentSession.metrics.errorRate > 20 ? '#cf1322' : '#3f8600',
              }}
            />
          </Col>
          <Col span={8}>
            <Statistic
              ...
              valueStyle={{
                color:
                  currentSession.metrics.comprehensionRate >= 90
                    ? '#3f8600'
                    : '#cf1322',
              }}
            />
        ...
        <div style={{ marginTop: 16 }}>
          ...
          <br />
          <Text>
            {currentSession.tasks[currentSession.currentTaskIndex]?.title}
          </Text>
        </div>
      </Card>
          ...
          {session.successCriteria.map((criterion, index) => (
            <div key={index} style={{ marginBottom: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <Text
                  strong
                  style={{ color: criterion.achieved ? '#3f8600' : '#cf1322' }}
                >
              ...
              <Progress
                percent={Math.min(
                  (criterion.actual / criterion.target) * 100,
                  100
                )}
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Statistic
                ...
                valueStyle={{
                  color:
                    session.metrics.comprehensionRate >= 90
                      ? '#3f8600'
                      : '#cf1322',
                }}
              />
            ...
            <Col span={12}>
              <Statistic
                ...
                valueStyle={{
                  color:
                    session.metrics.errorReduction >= 60
                      ? '#3f8600'
                      : '#cf1322',
                }}
              />
          ...
          <Row gutter={16} style={{ marginTop: 16 }}>
            <Col span={12}>
              <Statistic
                ...
                valueStyle={{
                  color:
                    session.metrics.visualIndicationClarity >= 100
                      ? '#3f8600'
                      : '#cf1322',
                }}
              />
            ...
            <Col span={12}>
              <Statistic
                ...
                valueStyle={{
                  color:
                    session.metrics.materaiUnderstanding >= 80
                      ? '#3f8600'
                      : '#cf1322',
                }}
              />
  ...
  const priceInheritanceTasks: UserTestingTask[] = [
    {
      ...
      description:
        'Pengguna harus memahami konsep pewarisan harga tanpa bantuan',
      instructions: [
      ...
      successCriteria: [
        'Memahami konsep inherit',
        'Memahami konsep custom',
        'Dapat menjelaskan kapan menggunakan masing-masing',
      ],
      maxDuration: 120,
      status: 'pending',
      errors: [],
    ...
    },
    {
      ...
      successCriteria: [
        'Mode berhasil diganti',
        'Harga berhasil dimasukkan',
        'Visual indikator terlihat jelas',
      ],
      maxDuration: 60,
      status: 'pending',
      errors: [],
    ...
    },
    {
      ...
      successCriteria: [
        'Melihat indikator penyimpangan',
        'Memahami warning',
    ...
    },
    {
      ...
      successCriteria: [
        'Melihat notifikasi materai',
        'Memahami threshold 5 juta',
        'Tahu cara menangani materai',
      ],
      maxDuration: 90,
      status: 'pending',
      errors: [],
...
function sendSessionResults(session: UserTestingSession): void {
  ...
  const totalErrors = session.tasks.reduce(
    (sum, task) => sum + task.errors.length,
    0
  )
