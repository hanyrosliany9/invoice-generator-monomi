...
export class UXMetricsCollector {
    ...
    return {
      ...
      markRenderComplete: () => {
        ...
        if (this.config.enableConsoleLogging) {
          console.log(`[UX Metrics] ${componentName} rendered in ${renderTime.toFixed(2)}ms`)
        }
      },
      ...
      trackInteraction: (interactionType: string, target?: string) => {
        ...
        return () => {
          ...
          if (this.config.enableConsoleLogging) {
            console.log(`[UX Metrics] ${componentName} ${interactionType} took ${interactionTime.toFixed(2)}ms`)
          }
        }
      },
  ...
  public trackBusinessJourneyPerformance(clientId: string): BusinessJourneyTimelinePerformance {
    ...
    this.sendMetricToAnalytics('business_journey_performance', businessJourneyPerformance, {
      clientId
    })
  ...
  private recordMetric(componentName: string, metric: UXMetrics): void {
    ...
    if (this.config.enableAnalytics && Math.random() < this.config.samplingRate) {
  ...
  private getComponentPerformance(componentName: string): UXMetrics | undefined {
    return this.metrics.get(componentName)
  }
  
  private getAverageInteractionTime(interactionType: string): number {
    const relevantInteractions = this.interactions.filter(i => i.type === interactionType)
    if (relevantInteractions.length === 0) return 0
    
    const totalTime = relevantInteractions.reduce((sum, i) => sum + i.duration, 0)
  ...
  private getRenderCount(componentName: string): number {
    ...
    return marks.filter(mark => mark.name.includes(`${componentName}-render-complete`)).length
  }
  
  private sendMetricToAnalytics(metricName: string, value: any, context?: any): void {
...
export const useUXMetrics = (componentName: string, config?: Partial<UXMetricsCollectorConfig>) => {
  const metricsCollector = useMemo(() => new UXMetricsCollector(config), [config])
  const trackerRef = useRef<ReturnType<typeof metricsCollector.trackComponentPerformance> | null>(null)
  
  useEffect(() => {
    trackerRef.current = metricsCollector.trackComponentPerformance(componentName)
