...
import { Card, Timeline, Button, Select, DatePicker, Input, Space, Alert, Spin, Empty } from 'antd'
import { 
  WhatsAppOutlined,
  WarningOutlined
...
export const BusinessJourneyTimeline: React.FC<BusinessJourneyTimelineProps> = ({
  ...
  const { trackInteraction, trackError } = useUXMetrics('BusinessJourneyTimeline')
  ...
  // Event handlers
  const handleEventClick = useCallback((event: BusinessJourneyEvent) => {
    const trackClick = trackInteraction('event_click', event.id)
    
    try {
      // Announce to screen reader
      if (!state.announcedEvents.has(event.id)) {
        announceToScreenReader(`Membuka detail ${getEventTitle(event.type, 'id')}`)
        setState(prev => ({
          ...prev,
  ...
  const handleFilterChange = useCallback((newFilters: Partial<BusinessJourneyFilters>) => {
  ...
  // WhatsApp sharing
  const handleWhatsAppShare = useCallback((event: BusinessJourneyEvent) => {
  ...
  const TimelineItem = useCallback(({ index, style }: { index: number, style: React.CSSProperties }) => {
  ...
  }, [filteredEvents, handleEventClick, handleWhatsAppShare, userPermissions.canViewFinancials])
  
  // Materai compliance warning
  const materiCompliance = useMemo(() => {
    if (!journeyData?.materaiCompliance) return null
    
    const { compliancePercentage, pendingAmount } = journeyData.materaiCompliance
      ...
      return (
        <Alert
          ...
          description={
            <div>
              <p>
                Terdapat invoice yang memerlukan materai sejumlah <strong>{formatIDR(pendingAmount)}</strong> 
                sesuai peraturan Indonesia.
              </p>
              <ul style={{ marginTop: 8, paddingLeft: 20 }}>
                <li>Materai harus ditempel pada dokumen fisik</li>
                <li>Materai dapat dibeli di kantor pos, bank, atau toko alat tulis</li>
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.target === timelineRef.current || timelineRef.current?.contains(e.target as Node)) {
        if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
          e.preventDefault()
          // Handle arrow key navigation between timeline items
          const buttons = timelineRef.current?.querySelectorAll(`.${styles['timelineDot']}`)
          if (!buttons) return
  ...
  if (!filteredEvents.length) {
    return (
      <Card {...(className && { className })}>
        ...
        >
          <p>Aktivitas bisnis akan muncul di sini setelah terjadi interaksi dengan klien.</p>
        </Empty>
      </Card>
    )
  }
  ...
  return (
    <div className={`${styles['businessJourneyContainer']} ${className || ''}`}>
      ...
      {/* Filters */}
      {showFilters && (
        <Card size="small" {...(styles['filtersCard'] && { className: styles['filtersCard'] })}>
          <Space wrap>
            <Input.Search
            ...
            <RangePicker
              value={[
                state.filters.dateRange.start ? dayjs(state.filters.dateRange.start) : null,
                state.filters.dateRange.end ? dayjs(state.filters.dateRange.end) : null
              ]}
              onChange={(dates) => {
                handleFilterChange({
                  dateRange: {
      ...
      {/* Timeline */}
      <Card {...(styles['timelineCard'] && { className: styles['timelineCard'] })}>
        <div 
          ref={timelineRef}
          role="region" 
        ...
        >
          <Timeline {...(styles['accessibleTimeline'] && { className: styles['accessibleTimeline'] })}>
        ...
        {journeyData && (
          <div className={styles['timelineFooter']}>
            <Space split={<span>•</span>}>
              <span>
                Total: {filteredEvents.length} aktivitas
              </span>
              {userPermissions.canViewFinancials && (
                <span>
                  Revenue: {formatIDR(journeyData.totalRevenue || 0)}
                </span>
              )}
              <span>
