  ...
  useEffect(() => {
    if (project) {
      const formData: ProjectFormData = {
        ...
        products: (project as any).products || [{ name: '', description: '', price: 0, quantity: 1 }],
      }
      ...
      if ((project as any).products) {
        const total = (project as any).products.reduce((sum: number, product: ProductItem) => {
      ...
      sidebar={
        <Space direction="vertical" size="large" style={{ width: '100%' }}>
          ...
          <Card size="small" title="Project Status">
            <Tag color={getStatusColor(form.getFieldValue('status'))} style={{ marginBottom: '8px' }}>
              {form.getFieldValue('status')}
            </Tag>
            <div>
              <Text type="secondary" style={{ fontSize: '12px' }}>
                Related: {project._count?.quotations || 0} quotations, {project._count?.invoices || 0} invoices
              </Text>
            </div>
          </Card>
