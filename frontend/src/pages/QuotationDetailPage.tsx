...
export const QuotationDetailPage: React.FC<QuotationDetailPageProps> = () => {
  ...
  const generateInvoiceMutation = useMutation({
    mutationFn: (quotationId: string) => quotationService.generateInvoice(quotationId),
    onSuccess: (data) => {
  ...
  const getStatusConfig = (status: Quotation['status']) => {
    const configs = {
      DRAFT: { color: 'default', icon: <EditOutlined />, text: 'Draft', progress: 20 },
      SENT: { color: 'blue', icon: <SendOutlined />, text: 'Sent', progress: 40 },
      APPROVED: { color: 'green', icon: <CheckCircleOutlined />, text: 'Approved', progress: 80 },
      DECLINED: { color: 'red', icon: <CloseCircleOutlined />, text: 'Declined', progress: 100 },
      REVISED: { color: 'orange', icon: <SyncOutlined />, text: 'Revised', progress: 60 },
    }
  ...
  const getValidityStatus = (validUntil: string) => {
    ...
    if (daysRemaining <= 3) return { status: 'expiring', color: 'orange', text: `${daysRemaining} days left` }
    return { status: 'valid', color: 'green', text: `${daysRemaining} days remaining` }
  }
  ...
  const getWorkflowTimeline = (quotation: Quotation) => {
    ...
    if (quotation.status === 'SENT' || quotation.status === 'APPROVED' || quotation.status === 'DECLINED') {
      timeline.push({
              ...
              <div>
                <Space align="center">
                  <Avatar 
                    ...
                    style={{ backgroundColor: statusConfig.color === 'default' ? '#d9d9d9' : statusConfig.color }}
                  />
                  <div>
            ...
            <div style={{ textAlign: 'center' }}>
              <Progress
                ...
                strokeColor={{
                  ...
                  '60%': '#faad14',
                  '80%': '#52c41a',
                  '100%': quotation.status === 'DECLINED' ? '#ff4d4f' : '#52c41a',
                }}
                format={(percent) => (
                  <span>
                    {statusConfig.icon}
            ...
            <Space direction="vertical" size="large" style={{ width: '100%' }}>
              <div>
                ...
                <div>
                  ...
                  <Text strong>{dayjs(quotation.date).format('DD MMM YYYY')}</Text>
                </div>
              </div>
              ...
              <div>
                ...
                <div>
                  <CalendarOutlined style={{ marginRight: '8px' }} />
                  <Text strong>{dayjs(quotation.validUntil).format('DD MMM YYYY')}</Text>
        ...
        <Col xs={12} sm={12} lg={6}>
          <Card>
            <Statistic
              ...
              valueStyle={{ 
                color: validityStatus.status === 'expired' ? '#ff4d4f' : 
                       validityStatus.status === 'expiring' ? '#faad14' : '#52c41a' 
              }}
            />
          </Card>
        </Col>
          ...
          items={[
            {
              ...
              children: (
                <div>
                  <Descriptions title="Quotation Information" bordered column={{ xs: 1, sm: 2 }}>
                    <Descriptions.Item label="Quotation Number">{quotation.quotationNumber}</Descriptions.Item>
                    <Descriptions.Item label="Status">
                      <Tag color={statusConfig.color} icon={statusConfig.icon}>
                        {statusConfig.text}
                      </Tag>
                    </Descriptions.Item>
                    <Descriptions.Item label="Created Date">{dayjs(quotation.date).format('DD MMM YYYY')}</Descriptions.Item>
                    <Descriptions.Item label="Valid Until">{dayjs(quotation.validUntil).format('DD MMM YYYY')}</Descriptions.Item>
                    <Descriptions.Item label="Total Amount">{formatIDR(quotation.totalAmount)}</Descriptions.Item>
                    <Descriptions.Item label="Amount Per Project">{formatIDR(quotation.amountPerProject)}</Descriptions.Item>
                    <Descriptions.Item label="Created By">{quotation.user?.name || 'Unknown'}</Descriptions.Item>
                    <Descriptions.Item label="Last Updated">{dayjs(quotation.updatedAt).format('DD MMM YYYY HH:mm')}</Descriptions.Item>
                  </Descriptions>
            ...
            },
            {
              ...
              children: (
                <div>
                  ...
                  {quotation.status === 'APPROVED' && quotation.invoices && quotation.invoices.length > 0 && (
                    <div style={{ marginTop: '24px' }}>
                        ...
                        {quotation.invoices.map((invoice) => (
                          <Card key={invoice.id} size="small">
                            <Space>
                              ...
                              <Button type="link" onClick={() => navigate(`/invoices/${invoice.id}`)}>
                                View Invoice
                              </Button>
                            </Space>
                          </Card>
                      ...
                      <Col xs={24} md={12}>
                        <Card title="Client Information" size="small">
                          <Descriptions column={1}>
                            <Descriptions.Item label="Name">{quotation.client.name}</Descriptions.Item>
                            <Descriptions.Item label="Company">{quotation.client.company}</Descriptions.Item>
                            <Descriptions.Item label="Email">{quotation.client.email}</Descriptions.Item>
                          </Descriptions>
                          <Button 
                            type="link" 
                            onClick={() => navigate(`/clients/${quotation.client!.id}`)}
                          >
                            View Client Details
                          </Button>
                        </Card>
                    ...
                    {quotation.project && (
                      <Col xs={24} md={12}>
                        <Card title="Project Information" size="small">
                          <Descriptions column={1}>
                            <Descriptions.Item label="Project Number">{quotation.project.number}</Descriptions.Item>
                            <Descriptions.Item label="Description">{quotation.project.description}</Descriptions.Item>
                            <Descriptions.Item label="Type">{quotation.project.type}</Descriptions.Item>
                          </Descriptions>
                          <Button 
                            type="link" 
                            onClick={() => navigate(`/projects/${quotation.project!.id}`)}
                          >
