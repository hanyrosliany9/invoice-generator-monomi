...
export const useMobileOptimized = () => {
  ...
  const detectDevice = useCallback(() => {
    ...
    const isTablet =
      /iPad|Android.*Tablet|Windows.*Touch/i.test(userAgent) && !isMobile
    ...
    if ('connection' in navigator) {
      ...
      if (connection.effectiveType) {
        connectionType = ['slow-2g', '2g'].includes(connection.effectiveType)
          ? 'slow'
          : 'fast'
      }
    }
    ...
    (baseStyles: React.CSSProperties = {}): React.CSSProperties => {
      ...
      return {
        ...
        fontSize: state.isMobile
          ? Math.max(16, parseInt(baseStyles.fontSize as string) || 16)
          : baseStyles.fontSize,
