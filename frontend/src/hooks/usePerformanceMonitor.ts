...
export const usePerformanceMonitor = (
  ...
  const activeMeasurements = useRef<
    Map<string, { name: string; startTime: number }>
  >(new Map())
  ...
  useEffect(() => {
    if (!enabled || typeof window === 'undefined' || Math.random() > sampleRate)
      return
  ...
  const checkPerformanceAlert = useCallback(
    (metricName: string, value: number, metric: PerformanceMetric) => {
  ...
  const recordPerformanceMetric = useCallback(
    (
      ...
      const metric: PerformanceMetric = {
        ...
        threshold:
          (DEFAULT_THRESHOLDS[name as keyof PerformanceThresholds] as number) ||
          1000,
        exceeded: false,
      }
      ...
      if (numericThreshold && duration > numericThreshold) {
        ...
        if (enableLogging) {
          console.warn(
            `Performance threshold exceeded: ${name} took ${duration.toFixed(2)}ms (threshold: ${numericThreshold}ms)`
          )
        }
      }
  ...
  const handleCustomMeasurement = useCallback(
  ...
  const getAverageTime = useCallback(
    (metricName: string): number => {
      const relevantMetrics = metrics.filter(
        m => m.name === metricName && !m.metadata?.['isEvent']
      )
  ...
  const getSlowOperations = useCallback(
    (threshold?: number): PerformanceMetric[] => {
      const defaultThreshold = threshold || 100
      return metrics
        .filter(m => m.duration > defaultThreshold && !m.metadata?.['isEvent'])
  ...
  const getOptimizationSuggestions = useCallback((): string[] => {
    ...
    const tableMetrics = metrics.filter(
      m => m.name.includes('table') || m.name.includes('render')
    )
    ...
    const apiMetrics = metrics.filter(
      m => m.name.includes('api') || m.name.includes('fetch')
    )
  ...
  return {
    ...
    recordMetric,
    recordError,
    recordBusinessEvent: (event: string, duration: number) =>
    ...
    exportReport: () =>
      JSON.stringify({ metrics, vitals, businessMetrics, alerts }, null, 2),
  }
}
