...
export const defaultShortcuts = {
  ...
  navigation: {
    goToInvoices: { key: 'alt+1', description: 'Go to Invoices', category: 'navigation' as const },
    goToClients: { key: 'alt+2', description: 'Go to Clients', category: 'navigation' as const },
    goToProjects: { key: 'alt+3', description: 'Go to Projects', category: 'navigation' as const },
    goToQuotations: { key: 'alt+4', description: 'Go to Quotations', category: 'navigation' as const },
    goToDashboard: { key: 'alt+0', description: 'Go to Dashboard', category: 'navigation' as const },
    goToSettings: { key: 'alt+9', description: 'Go to Settings', category: 'navigation' as const },
  },
  
  // Action shortcuts
  actions: {
    createNew: { key: 'ctrl+n', description: 'Create New Item', category: 'actions' as const },
    save: { key: 'ctrl+s', description: 'Save Current Form', category: 'actions' as const },
    search: { key: 'ctrl+k', description: 'Focus Search', category: 'actions' as const },
    refresh: { key: 'f5', description: 'Refresh Page', category: 'actions' as const },
    export: { key: 'ctrl+e', description: 'Export Data', category: 'actions' as const },
    print: { key: 'ctrl+p', description: 'Print Document', category: 'actions' as const },
  },
  
  // Editing shortcuts
  editing: {
    selectAll: { key: 'ctrl+a', description: 'Select All Items', category: 'editing' as const },
    copy: { key: 'ctrl+c', description: 'Copy Selected', category: 'editing' as const },
    paste: { key: 'ctrl+v', description: 'Paste', category: 'editing' as const },
    undo: { key: 'ctrl+z', description: 'Undo Last Action', category: 'editing' as const },
    redo: { key: 'ctrl+y', description: 'Redo Action', category: 'editing' as const },
  },
  
  // General shortcuts
  general: {
    help: { key: 'f1', description: 'Show Keyboard Shortcuts', category: 'general' as const },
    closeModal: { key: 'escape', description: 'Close Modal/Dialog', category: 'general' as const },
    confirm: { key: 'enter', description: 'Confirm Action', category: 'general' as const },
    previousPage: { key: 'alt+left', description: 'Previous Page', category: 'general' as const },
    nextPage: { key: 'alt+right', description: 'Next Page', category: 'general' as const },
  }
};
...
const shouldIgnoreShortcut = (target: EventTarget | null): boolean => {
  ...
  return isInput || isContentEditable || (isFormElement && ['input', 'textarea'].includes(tagName));
};
...
export const useKeyboardShortcuts = ({
  ...
  const handleKeyDown = useCallback((event: KeyboardEvent) => {
    ...
    const matchedShortcut = shortcuts.find(s => s.key === shortcutKey && s.enabled !== false);
    ...
    if (matchedShortcut) {
      ...
      try {
        ...
        if (showTooltips && shortcutKey !== 'f1') { // Don't show tooltip for help
          message.success({
