...
export interface IndonesianCulturalPreferences {
  ...
  region: 'jakarta' | 'jawa' | 'sumatera' | 'kalimantan' | 'sulawesi' | 'papua' | 'bali_ntt_ntb' | 'maluku' | 'general'
  timezone: string
...
export interface IndonesianCulturalUXContextType {
  ...
  // Actions
  updatePreferences: (preferences: Partial<IndonesianCulturalPreferences>) => void
  updateWorkflow: (workflow: Partial<IndonesianBusinessWorkflow>) => void
  
  // Helpers
  formatCurrency: (amount: number, style?: 'full' | 'abbreviated' | 'symbol-only') => string
  formatDateTime: (date: Date, style?: 'indonesian' | 'international' | 'mixed') => string
  getGreeting: (timeOfDay?: 'morning' | 'afternoon' | 'evening', formal?: boolean) => string
  getHonorific: (gender?: 'male' | 'female', context?: 'business' | 'general') => string
  getBusinessPhrase: (type: keyof IndonesianCulturalMessages['businessPhrases'], context?: any) => string
  
  // WhatsApp integration helpers
  formatWhatsAppMessage: (template: string, data: Record<string, any>) => string
  getWhatsAppBusinessTemplate: (type: 'quotation' | 'invoice' | 'reminder' | 'follow-up') => string
  
  // Cultural validation
  validateBusinessEtiquette: (action: string, context: any) => { valid: boolean; suggestions: string[] }
...
const INDONESIAN_MESSAGES: IndonesianCulturalMessages = {
  ...
  businessPhrases: {
    ...
    invoiceReminder: 'Pengingat Invoice #{{invoiceNumber}} - Jatuh Tempo {{dueDate}}',
    paymentRequest: 'Mohon konfirmasi pembayaran untuk Invoice #{{invoiceNumber}}',
  ...
  culturalNotes: {
    ...
    nationalHolidays: [
      'Tahun Baru Masehi', 'Imlek', 'Nyepi', 'Wafat Isa Al-Masih',
      'Hari Raya Idul Fitri', 'Hari Buruh', 'Kenaikan Isa Al-Masih',
      'Hari Raya Waisak', 'Hari Lahir Pancasila', 'Hari Raya Idul Adha',
      'Tahun Baru Islam', 'Maulid Nabi Muhammad', 'Hari Kemerdekaan',
      'Hari Natal'
    ],
    regionalCustoms: [
...
const IndonesianCulturalUXContext = createContext<IndonesianCulturalUXContextType | null>(null)

export const IndonesianCulturalUXProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const { i18n } = useTranslation()
  
  // State management
  const [preferences, setPreferences] = useState<IndonesianCulturalPreferences>(DEFAULT_PREFERENCES)
  const [workflow, setWorkflow] = useState<IndonesianBusinessWorkflow>(DEFAULT_WORKFLOW)
  ...
  useEffect(() => {
    const savedPreferences = localStorage.getItem('indonesian-cultural-preferences')
    if (savedPreferences) {
      try {
  ...
  // Update preferences
  const updatePreferences = useCallback((newPreferences: Partial<IndonesianCulturalPreferences>) => {
    setPreferences(prev => {
      const updated = { ...prev, ...newPreferences }
      localStorage.setItem('indonesian-cultural-preferences', JSON.stringify(updated))
      return updated
    })
  }, [])

  // Update workflow
  const updateWorkflow = useCallback((newWorkflow: Partial<IndonesianBusinessWorkflow>) => {
    setWorkflow(prev => ({ ...prev, ...newWorkflow }))
  }, [])

  // Currency formatting based on Indonesian preferences
  const formatCurrency = useCallback((amount: number, style?: 'full' | 'abbreviated' | 'symbol-only') => {
  ...
  const formatDateTime = useCallback((date: Date, style?: 'indonesian' | 'international' | 'mixed') => {
  ...
  const getGreeting = useCallback((timeOfDay?: 'morning' | 'afternoon' | 'evening', formal?: boolean) => {
    const useFormal = formal !== undefined ? formal : preferences.languageFormality === 'formal'
  ...
  const getHonorific = useCallback((gender?: 'male' | 'female', context?: 'business' | 'general') => {
  ...
  const getBusinessPhrase = useCallback((type: keyof IndonesianCulturalMessages['businessPhrases'], context?: any) => {
  ...
  const getWhatsAppBusinessTemplate = useCallback((type: 'quotation' | 'invoice' | 'reminder' | 'follow-up') => {
    const templates = {
...
Mohon informasi terkait hal tersebut. Terima kasih atas perhatiannya.`
  ...
  const validateBusinessEtiquette = useCallback((action: string, context: any) => {
    ...
    if (action === 'send_invoice' || action === 'send_reminder') {
      ...
      if (sendTime.getDay() === 5 && hour >= 11 && hour <= 13) {
        valid = false
        suggestions.push('Hindari pengiriman saat waktu sholat Jumat (11:30-13:00)')
      }
      
      // Respect business hours
      if (hour < 8 || hour > 17) {
        suggestions.push('Pertimbangkan mengirim dalam jam kerja (08:00-17:00 WIB)')
      }
    }
    
    // Check formality level
    if (action === 'send_message' && context.message) {
      if (!context.message.includes('Bapak') && !context.message.includes('Ibu')) {
        suggestions.push('Gunakan sapaan formal "Bapak/Ibu" untuk komunikasi bisnis')
      }
    }
    
    // Check payment reminder tone
    if (action === 'send_reminder' && context.reminderCount > 2) {
      suggestions.push('Gunakan bahasa yang lebih halus untuk pengingatan berulang')
    }
  ...
  const getCulturalRecommendations = useCallback(() => {
    ...
    if (preferences.languageFormality === 'informal') {
      recommendations.push('Pertimbangkan menggunakan bahasa formal untuk komunikasi bisnis Indonesia')
    }
    
    // WhatsApp integration
    if (preferences.whatsappIntegrationLevel === 'basic') {
      recommendations.push('Tingkatkan integrasi WhatsApp untuk komunikasi yang lebih efektif dengan klien Indonesia')
    }
    
    // Regional considerations
    if (preferences.region !== 'general') {
      recommendations.push(`Aktifkan fitur khusus untuk wilayah ${preferences.region}`)
    }
    
    // Mobile optimization
    if (!preferences.mobileFirst) {
      recommendations.push('Aktifkan optimisasi mobile-first untuk pengalaman yang lebih baik di Indonesia')
    }
...
export const useIndonesianCulturalUX = (): IndonesianCulturalUXContextType => {
  ...
  if (!context) {
    throw new Error('useIndonesianCulturalUX must be used within IndonesianCulturalUXProvider')
  }
  return context
}
