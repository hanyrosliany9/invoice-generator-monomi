...
export class FeatureFlagsController {
   ...
   */
  @Get('user-flags')
  @ApiOperation({ summary: 'Get feature flags for current user with Indonesian business context' })
  ...
  async getUserFeatureFlags(
    ...
    try {
      ...
      const flags = await this.featureFlagsService.getUserFeatureFlags(userContext, query.flagIds);
  ...
  @Post()
  @Roles('admin')
  @ApiOperation({ summary: 'Create new feature flag for Indonesian business context' })
  ...
  async createFeatureFlag(
    ...
    try {
      ...
      const contextValidation = await this.featureFlagsService.validateIndonesianBusinessContext(createDto);
      ...
      const flag = await this.featureFlagsService.createFeatureFlag(createDto, user.id);
      
      return {
        success: true,
        data: flag,
        message: 'Feature flag created for Indonesian Business Management System'
      };
    } catch (error) {
      if (error instanceof HttpException) {
        throw error;
  ...
  async updateFeatureFlag(
    ...
    try {
      const flag = await this.featureFlagsService.updateFeatureFlag(flagId, updateDto, user.id);
  ...
  @Post(':flagId/enable')
  @Roles('admin', 'manager')
  @ApiOperation({ summary: 'Enable feature flag with Indonesian business safety checks' })
  @ApiResponse({ status: 200, description: 'Feature flag enabled successfully' })
  async enableFeatureFlag(
    ...
    try {
      ...
      const safetyCheck = await this.featureFlagsService.performSafetyChecks(flagId);
      ...
      const result = await this.featureFlagsService.enableFeatureFlag(flagId, rolloutConfig, user.id);
  ...
  @ApiResponse({ status: 200, description: 'Feature flag disabled successfully' })
  ...
  @ApiResponse({ status: 200, description: 'Rollout percentage updated successfully' })
  ...
  @Get(':flagId/stats')
  @Roles('admin', 'manager')
  @ApiOperation({ summary: 'Get feature flag statistics for Indonesian business analytics' })
  @ApiResponse({ status: 200, description: 'Feature flag statistics retrieved' })
  async getFeatureStats(@Param('flagId') flagId: string): Promise<FeatureFlagStatsDto> {
    try {
  ...
  async getRolloutHealth(@Param('flagId') flagId: string) {
    try {
      ...
      return {
        success: true,
        data: health,
        recommendations: await this.featureFlagsService.generateHealthRecommendations(flagId)
      };
    } catch (error) {
      throw new HttpException(
  ...
  async emergencyRollback(
    @Param('flagId') flagId: string,
    @Body() body: { reason: string; severity: 'low' | 'medium' | 'high' | 'critical' },
    @User() user: any
  ) {
    try {
      ...
      await this.featureFlagsService.sendEmergencyAlert(flagId, body.reason, body.severity);
  ...
  @Roles('admin', 'manager')
  @ApiOperation({ summary: 'Get Indonesian business context validation results' })
  @ApiResponse({ status: 200, description: 'Indonesian validation results retrieved' })
  async getIndonesianValidation(@Param('flagId') flagId: string) {
    try {
      const validation = await this.featureFlagsService.getIndonesianBusinessValidation(flagId);
  ...
  async getAuditLog(
    @Param('flagId') flagId: string,
    @Query() query: { limit?: number; offset?: number; startDate?: string; endDate?: string }
  ) {
    try {
  ...
  @Post(':flagId/test')
  @Roles('admin', 'manager')
  @ApiOperation({ summary: 'Test feature flag configuration before deployment' })
  @ApiResponse({ status: 200, description: 'Feature flag test completed' })
  async testFeatureFlag(
    @Param('flagId') flagId: string,
    @Body() testConfig: { testUsers?: string[]; testRegions?: string[]; dryRun?: boolean }
  ) {
    try {
      const testResult = await this.featureFlagsService.testFeatureFlag(flagId, testConfig);
