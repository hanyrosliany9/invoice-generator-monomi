...
export function handleServiceError(error: unknown, operation: string, entityType: string = 'data'): never {
  ...
  if (error instanceof Prisma.PrismaClientKnownRequestError) {
    switch (error.code) {
      case 'P2002':
        throw new ConflictException(`${entityType} dengan data tersebut sudah ada`);
      case 'P2025':
      ...
      default:
        throw new InternalServerErrorException(`Terjadi kesalahan database: ${error.message}`);
    }
  }
  ...
  throw new InternalServerErrorException(`Terjadi kesalahan pada ${operation}: ${message}`);
}
...
export function validateIndonesianBusinessRules(data: any): void {
  ...
  if (data.totalAmount && typeof data.totalAmount === 'number' && data.totalAmount > 5000000) {
