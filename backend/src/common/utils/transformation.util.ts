...
export class TransformationUtil {
   ...
   */
  static generateIndonesianReference(type: 'invoice' | 'quotation' | 'receipt', sequence: number): string {
    const year = new Date().getFullYear()
    const month = String(new Date().getMonth() + 1).padStart(2, '0')
  ...
  static numberToIndonesianWords(num: number): string {
    ...
    if (num < 1000) return ones[Math.floor(num / 100)] + ' ratus' + (num % 100 ? ' ' + this.numberToIndonesianWords(num % 100) : '')
    ...
    for (let i = 0; i < scales.length; i++) {
      ...
      if (num < scale) {
        ...
        return this.numberToIndonesianWords(quotient) + ' ' + scales[i] + (remainder ? ' ' + this.numberToIndonesianWords(remainder) : '')
      }
    }
