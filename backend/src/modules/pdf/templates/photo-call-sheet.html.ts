  ...
  const shotsHtml =
    cs.shots
      ?.map(
        (shot: any) => `
    <tr>
      <td style="text-align: center; font-weight: bold;">${escapeHtml(shot.shotNumber)}</td>
      <td>${escapeHtml(shot.shotName) || ""}</td>
      <td>${escapeHtml(shot.lookReference) || ""}</td>
      <td>${escapeHtml(shot.setupLocation) || ""}</td>
      <td style="text-align: center;">${escapeHtml(shot.estStartTime) || "-"}</td>
      <td style="text-align: center;">${escapeHtml(shot.estDuration) || "-"}</td>
      <td style="font-size: 9px;">${escapeHtml(shot.wardrobeNotes) || ""}</td>
    </tr>
  `,
      )
      .join("") || "";
  ...
  const modelsHtml =
    cs.models
      ?.map(
        (model: any) => `
    <tr>
      <td style="text-align: center;">${escapeHtml(model.modelNumber) || ""}</td>
      <td style="font-weight: bold;">${escapeHtml(model.modelName)}</td>
      <td>${escapeHtml(model.agencyName) || "-"}</td>
      <td style="font-size: 9px;">${model.arrivalType === "CAMERA_READY" ? "Camera Ready" : "Styled"}</td>
      <td style="text-align: center;">${escapeHtml(model.arrivalTime)}</td>
      <td style="text-align: center;">${escapeHtml(model.hmuStartTime) || "-"}</td>
      <td style="text-align: center;">${escapeHtml(model.cameraReadyTime) || "-"}</td>
      <td>${escapeHtml(model.hmuArtist) || "-"}</td>
    </tr>
  `,
      )
      .join("") || "";
  ...
  const wardrobeHtml =
    cs.wardrobe
      ?.map(
        (item: any) => `
    <tr>
      <td style="font-weight: bold;">${escapeHtml(item.itemName)}</td>
      <td style="text-align: center;">${escapeHtml(item.size) || "-"}</td>
      <td>${escapeHtml(item.color) || "-"}</td>
      <td>${escapeHtml(item.forModel) || "-"}</td>
      <td>${escapeHtml(item.forShot) || "-"}</td>
      <td style="font-size: 9px;">${escapeHtml(item.status)}</td>
    </tr>
  `,
      )
      .join("") || "";
  ...
  const hmuHtml =
    cs.hmuSchedule
      ?.map(
        (hmu: any) => `
    <tr>
      <td>${escapeHtml(hmu.artistName)}</td>
      <td style="font-size: 9px;">${hmu.artistRole === "KEY_STYLIST" ? "Key Stylist" : escapeHtml(hmu.artistRole)}</td>
      <td style="text-align: center;">${escapeHtml(hmu.stationNumber) || "-"}</td>
      <td style="text-align: center;">${escapeHtml(hmu.callTime)}</td>
      <td style="text-align: center;">${escapeHtml(hmu.availableFrom) || "-"}</td>
      <td style="text-align: center;">${escapeHtml(hmu.availableUntil) || "-"}</td>
      <td style="font-size: 9px;">${escapeHtml(hmu.assignedModels) || "-"}</td>
    </tr>
  `,
      )
      .join("") || "";
...
<body>
  <div class="page-container">
      ...
      <div class="header-top">
        ...
        <div class="header-left">
          ...
          <div class="production-info-box">
            <div><span class="production-info-label">Photographer:</span> ${escapeHtml(cs.photographer) || "-"}</div>
            <div><span class="production-info-label">Art Director:</span> ${escapeHtml(cs.artDirector) || "-"}</div>
            <div><span class="production-info-label">Stylist:</span> ${escapeHtml(cs.stylist) || "-"}</div>
            <div><span class="production-info-label">H&MU Lead:</span> ${escapeHtml(cs.hmuLead) || "-"}</div>
          </div>
        </div>

        <!-- Center: Title -->
        <div class="header-center">
          <div class="call-sheet-title">${escapeHtml(cs.productionName) || "PHOTO CALL SHEET"}</div>
        ...
        <div class="header-right">
          <table class="key-times-table">
            ...
            <tr>
              ...
              <td class="key-times-value">${escapeHtml(cs.sessionType) || "—"}</td>
            </tr>
          </table>
        </div>
      </div>
    ...
    <div class="logistics-section">
      <div class="logistics-grid">
        <div class="logistics-box">
          ...
          <div class="logistics-value">${escapeHtml(cs.locationName) || ""}<br/><small>${escapeHtml(cs.locationAddress) || ""}</small></div>
        </div>
        <div class="logistics-box">
          <div class="logistics-label">CLIENT</div>
          <div class="logistics-value">${escapeHtml(cs.clientName) || ""}<br/><small>${escapeHtml(cs.clientContact) || ""}</small></div>
        </div>
        <div class="logistics-box">
          <div class="logistics-label">AGENCY</div>
          <div class="logistics-value">${escapeHtml(cs.agencyName) || ""}</div>
        </div>
        <div class="logistics-box">
          <div class="logistics-label">PARKING</div>
          <div class="logistics-value">${escapeHtml(cs.crewParking || cs.parkingNotes) || ""}</div>
        </div>
        <div class="logistics-box">
        ...
        <div class="logistics-box">
          ...
          <div class="logistics-value">${escapeHtml(cs.sunrise) || "—"}</div>
        </div>
        <div class="logistics-box">
          <div class="logistics-label">SUNSET</div>
          <div class="logistics-value">${escapeHtml(cs.sunset) || "—"}</div>
        </div>
      </div>
    </div>
    ...
    <div class="notes-section">
      ...
      <div class="notes-content">${escapeHtml(cs.productionNotes || cs.generalNotes) || ""}</div>
    </div>
    `
        : ""
    }
    ...
    <div class="footer-section">
      Generated: ${new Date().toLocaleString()} | ${escapeHtml(cs.productionName) || "Photo Call Sheet"} | Day ${cs.dayNumber || 1}
    </div>
  </div>
</body>
</html>`;
