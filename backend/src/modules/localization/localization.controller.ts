...
export class LocalizationController {
  ...
  async getPaymentMethods(
    ...
    try {
      ...
      if (type) {
        paymentMethods = this.localizationService.getPaymentMethodsByType(
          type as any,
        );
      } else {
        paymentMethods = this.localizationService.getPaymentMethods(
  ...
  async getPopularPaymentMethods() {
    try {
      const paymentMethods =
        this.localizationService.getPopularPaymentMethods();
      return {
        data: paymentMethods,
  ...
  async formatCurrency(@Body() formatCurrencyDto: FormatCurrencyDto) {
    try {
      const formatted = this.localizationService.formatCurrency(
        formatCurrencyDto.amount,
      );
      const inWords = this.localizationService.formatAmountInWords(
        formatCurrencyDto.amount,
      );
  ...
  async calculateVAT(@Body() calculateVATDto: CalculateVATDto) {
    try {
      const vatAmount = this.localizationService.calculateVAT(
        calculateVATDto.amount,
      );
      const totalWithVAT = this.localizationService.calculateTotalWithVAT(
        calculateVATDto.amount,
      );
      ...
      return {
        data: {
          ...
          totalWithVAT,
          vatRate:
            this.localizationService.getIndonesianBusinessConfig().vatRate,
        },
  ...
  async validateBusinessData(
    @Body() validateBusinessDataDto: ValidateBusinessDataDto,
  ) {
    try {
      const results = {
        phone: validateBusinessDataDto.phone
          ? this.localizationService.validateIndonesianPhone(
              validateBusinessDataDto.phone,
            )
          : null,
        npwp: validateBusinessDataDto.npwp
          ? this.localizationService.validateNPWP(validateBusinessDataDto.npwp)
          : null,
        formattedNPWP: validateBusinessDataDto.npwp
          ? this.localizationService.formatNPWP(validateBusinessDataDto.npwp)
          : null,
      };
  ...
  async checkWorkingDay(@Param("date") dateStr: string) {
    try {
      ...
      const isBankHoliday =
        this.localizationService.isIndonesianBankHoliday(date);
