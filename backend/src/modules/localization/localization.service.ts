...
export class LocalizationService {
    ...
    this.paymentMethods = [
      {
        ...
        instructions: 'Transfer ke rekening BCA berikut dan konfirmasi pembayaran via WhatsApp',
      ...
      },
      {
        ...
        instructions: 'Transfer ke rekening Mandiri berikut dan konfirmasi pembayaran via WhatsApp',
        fees: { fixed: 0, percentage: 0 },
        available: true,
        popularInIndonesia: true
      },
      {
        ...
        instructions: 'Transfer ke rekening BRI berikut dan konfirmasi pembayaran via WhatsApp',
        fees: { fixed: 0, percentage: 0 },
        available: true,
        popularInIndonesia: true
      },
      {
        ...
        instructions: 'Transfer ke rekening BNI berikut dan konfirmasi pembayaran via WhatsApp',
        fees: { fixed: 0, percentage: 0 },
        available: true,
        popularInIndonesia: true
      },
      {
        ...
        instructions: 'Transfer via GoPay ke nomor berikut dan screenshot bukti pembayaran',
        fees: { fixed: 0, percentage: 0 },
        available: true,
        popularInIndonesia: true
      },
      {
        ...
        instructions: 'Transfer via OVO ke nomor berikut dan screenshot bukti pembayaran',
        fees: { fixed: 0, percentage: 0 },
        available: true,
        popularInIndonesia: true
      },
      {
        ...
        instructions: 'Transfer via DANA ke nomor berikut dan screenshot bukti pembayaran',
        fees: { fixed: 0, percentage: 0 },
        available: true,
        popularInIndonesia: true
      },
  ...
  getPaymentMethods(includeUnavailable = false): PaymentMethod[] {
    return this.paymentMethods.filter(method => includeUnavailable || method.available);
  }
  ...
  getPopularPaymentMethods(): PaymentMethod[] {
    return this.paymentMethods.filter(method => method.popularInIndonesia && method.available);
  }
  ...
  getPaymentMethodsByType(type: PaymentMethod['type']): PaymentMethod[] {
    return this.paymentMethods.filter(method => method.type === type && method.available);
  }
  ...
  calculateTotalWithVAT(amount: number): number {
    return TransformationUtil.calculateTotalWithVAT(amount, this.config.vatRate);
  }
  ...
  isWorkingDay(date: Date): boolean {
    ...
    return this.config.workingDays.includes(dayOfWeek) && !this.isIndonesianBankHoliday(date);
  }
  ...
  generateQuotationNumber(sequence: number): string {
    return TransformationUtil.generateIndonesianReference('quotation', sequence);
  }
  ...
  getDocumentTemplates(): {
    ...
    return {
      invoice: 'Terima kasih atas kepercayaan Anda untuk menggunakan layanan kami.',
      quotation: 'Penawaran ini berlaku selama 30 hari sejak tanggal penawaran.',
