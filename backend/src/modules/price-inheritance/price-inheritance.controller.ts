...
export class PriceInheritanceController {
  ...
  @ApiOperation({
    ...
    description:
      "Retrieve available price sources for a given entity, supporting Indonesian business workflow from project → quotation → invoice",
  })
  @ApiQuery({
    name: "entityType",
    enum: EntityType,
  ...
  async getAvailableSources(
    ...
    this.logger.log(
      `Getting price sources for ${query.entityType}:${query.entityId} by user ${req.user.sub}`,
    );
    ...
    try {
      ...
      this.logger.log(
        `Found ${sources.length} price sources for ${query.entityType}:${query.entityId}`,
      );
      return sources;
    } catch (error) {
      this.logger.error(
        `Failed to get price sources: ${getErrorMessage(error)}`,
  ...
  @ApiOperation({
    ...
    description:
      "Validate price inheritance with Indonesian business rules including materai compliance, tax requirements, and business etiquette",
  })
  @ApiBody({
  ...
  async validatePriceInheritance(
    ...
    this.logger.log(
      `Validating price inheritance: ${dto.amount} IDR in ${dto.mode} mode by user ${req.user.sub}`,
    );

    try {
      const validation =
        await this.priceInheritanceService.validatePriceInheritance(
          dto.amount,
          dto.mode,
          dto.sourceId,
          dto.inheritedAmount,
      ...
      this.logger.log(
        `Price validation completed: valid=${validation.isValid}, errors=${validation.errors.length}, warnings=${validation.warnings.length}`,
      );
      return validation;
    } catch (error) {
      this.logger.error(
        `Price validation failed: ${getErrorMessage(error)}`,
        isError(error) ? error.stack : undefined,
      );
      throw error;
    }
  ...
  @ApiOperation({
    ...
    description:
      "Create a new price inheritance configuration with Indonesian compliance validation and user interaction tracking",
  })
  @ApiBody({
  ...
  async createPriceInheritance(
    ...
    this.logger.log(
      `Creating price inheritance for ${dto.entityType}:${dto.entityId} by user ${req.user.sub}`,
    );
    ...
    try {
      ...
      this.logger.log(
        `Price inheritance created successfully for ${dto.entityType}:${dto.entityId}`,
      );
      return result;
    } catch (error) {
      this.logger.error(
        `Failed to create price inheritance: ${getErrorMessage(error)}`,
        isError(error) ? error.stack : undefined,
      );
      throw error;
    }
  ...
  @ApiOperation({
    ...
    description:
      "Update an existing price inheritance configuration with re-validation of Indonesian business rules",
  })
  @ApiParam({
    name: "id",
  ...
  async updatePriceInheritance(
    ...
    this.logger.log(
      `Updating price inheritance ${id} for ${dto.entityType}:${dto.entityId} by user ${req.user.sub}`,
    );
    ...
    try {
      ...
      this.logger.error(
        `Failed to update price inheritance ${id}: ${getErrorMessage(error)}`,
        isError(error) ? error.stack : undefined,
      );
      throw error;
    }
  ...
  @ApiOperation({
    ...
    description:
      "Retrieve analytics data for price inheritance usage, compliance rates, and user behavior patterns",
  })
  @ApiQuery({
    name: "dateFrom",
    type: Date,
  ...
  async getPriceInheritanceAnalytics(
    ...
    this.logger.log(
      `Getting price inheritance analytics by user ${req.user.sub}`,
    );
    ...
    try {
      ...
      const analytics =
        await this.priceInheritanceService.getPriceInheritanceAnalytics(
      ...
      this.logger.error(
        `Failed to get price inheritance analytics: ${getErrorMessage(error)}`,
        isError(error) ? error.stack : undefined,
      );
      throw error;
    }
  ...
  @ApiOperation({
    ...
    description:
      "Check the health and availability of the price inheritance service",
  })
  @ApiResponse({
    status: HttpStatus.OK,
  ...
  @ApiOperation({
    ...
    description:
      "Calculate the required materai (stamp duty) amount based on Indonesian regulations (UU No. 13 Tahun 1985)",
  })
  @ApiBody({
    schema: {
      type: "object",
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        amount: {
          type: "number",
          description: "Required materai amount in IDR",
        },
        regulation: {
          type: "string",
          description: "Reference to Indonesian regulation",
        },
        guidance: {
          type: "string",
          description: "Guidance for materai application",
        },
      },
    },
  })
  ...
  @ApiOperation({
    ...
    description:
      "Get business etiquette recommendations based on transaction amount and Indonesian business culture",
  })
  @ApiParam({
    name: "amount",
    type: "number",
  ...
  async getBusinessEtiquette(
    ...
    this.logger.log(
      `Getting business etiquette guidance for ${amount} IDR by user ${req.user.sub}`,
    );
    ...
    if (hour >= 9 && hour <= 12) {
      suggestedTiming =
        "Pagi ini (09:00-12:00 WIB) - Waktu ideal untuk diskusi bisnis";
    } else if (hour >= 13 && hour <= 16) {
      suggestedTiming =
        "Siang ini (13:00-16:00 WIB) - Waktu baik untuk negosiasi";
    }
