...
describe("PriceInheritanceService", () => {
  ...
  describe("validatePriceInheritance", () => {
    ...
    it("should provide business timing guidance", async () => {
      ...
      expect(result.compliance.businessEtiquette.communicationStyle).toMatch(
        /formal|semi-formal|casual/,
      );
      expect(result.compliance.businessEtiquette.culturalNotes).toBeInstanceOf(
        Array,
      );
      expect(
        result.compliance.businessEtiquette.culturalNotes.length,
      ).toBeGreaterThan(0);
    });
  });
  ...
  describe("createPriceInheritance", () => {
    ...
    it("should create price inheritance successfully", async () => {
      ...
      const result = await service.createPriceInheritance(
        createDto,
        "user-123",
      );
    ...
    it("should handle interaction tracking errors gracefully", async () => {
      mockPrismaService.userInteraction.create.mockRejectedValue(
  ...
  describe("Indonesian Compliance", () => {
    it("should calculate correct materai for different amounts", async () => {
      ...
      for (const testCase of testCases) {
        ...
        expect(result.compliance.materaiRequired).toBe(
          testCase.expectedMaterai > 0,
        );
      }
    });
    ...
    it("should provide appropriate communication style based on amount", async () => {
      ...
      const smallResult = await service.validatePriceInheritance(
        50000000,
        PriceInheritanceMode.INHERIT,
      );
      expect(smallResult.compliance.businessEtiquette.communicationStyle).toBe(
        "semi-formal",
      );

      // Large transaction - formal
      const largeResult = await service.validatePriceInheritance(
        150000000,
        PriceInheritanceMode.INHERIT,
      );
      expect(largeResult.compliance.businessEtiquette.communicationStyle).toBe(
        "formal",
      );
    });
  ...
  describe("Error Handling", () => {
    it("should handle NaN and Infinity amounts", async () => {
      const nanResult = await service.validatePriceInheritance(
        NaN,
        PriceInheritanceMode.INHERIT,
      );
      expect(nanResult.isValid).toBe(false);

      const infinityResult = await service.validatePriceInheritance(
        Infinity,
        PriceInheritanceMode.INHERIT,
      );
  ...
  describe("Performance", () => {
    it("should validate quickly for typical amounts", async () => {
      ...
      await service.validatePriceInheritance(
        50000000,
        PriceInheritanceMode.INHERIT,
      );
    ...
    it("should handle multiple concurrent validations", async () => {
      const promises = Array.from({ length: 10 }, (_, i) =>
        service.validatePriceInheritance(
          (i + 1) * 10000000,
          PriceInheritanceMode.INHERIT,
        ),
      );
