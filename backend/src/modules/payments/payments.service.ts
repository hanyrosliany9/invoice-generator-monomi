...
export class PaymentsService {
  ...
  async create(createPaymentDto: CreatePaymentDto): Promise<PaymentResponseDto> {
    ...
    const existingPayments = invoice.payments.reduce((sum, payment) => {
      return sum + (payment.status === PaymentStatus.CONFIRMED ? Number(payment.amount) : 0)
    }, 0)
  ...
  async update(id: string, updatePaymentDto: UpdatePaymentDto): Promise<PaymentResponseDto> {
    ...
    if (updatePaymentDto.status === PaymentStatus.CONFIRMED && !updatePaymentDto.confirmedAt) {
  ...
  private async updateInvoiceStatus(invoiceId: string): Promise<void> {
    ...
    const confirmedPayments = invoice.payments.filter(p => p.status === PaymentStatus.CONFIRMED)
    const totalPaid = confirmedPayments.reduce((sum, payment) => sum + Number(payment.amount), 0)
    const totalAmount = Number(invoice.totalAmount)
