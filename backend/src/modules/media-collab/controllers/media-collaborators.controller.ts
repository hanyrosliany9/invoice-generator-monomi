import { Controller, UseGuards, Get, Post, Put, Delete, Param, Body, Request } from '@nestjs/common';
...
export class MediaCollaboratorsController {
  constructor(private readonly collaboratorsService: MediaCollaboratorsService) {}
  ...
  async addCollaborator(
    ...
    return this.collaboratorsService.addCollaborator(projectId, dto, req.user.id);
  }

  @Put('project/:projectId/:collaboratorId')
  @ApiOperation({ summary: 'Update collaborator role' })
  @ApiResponse({ status: 200, description: 'Collaborator updated successfully' })
  @ApiResponse({ status: 400, description: 'Cannot change project creator role' })
  @ApiResponse({ status: 403, description: 'Permission denied' })
  ...
  @ApiResponse({ status: 201, description: 'Guest invited successfully, invite link returned' })
  @ApiResponse({ status: 400, description: 'Guest already invited to this project' })
  @ApiResponse({ status: 403, description: 'Permission denied (must be OWNER or EDITOR)' })
  @ApiResponse({ status: 404, description: 'Project not found' })
  async inviteGuest(
    @Param('projectId') projectId: string,
  ...
  @ApiResponse({ status: 200, description: 'Guest access revoked successfully' })
  @ApiResponse({ status: 400, description: 'Cannot revoke internal user access' })
  @ApiResponse({ status: 403, description: 'Permission denied (must be OWNER)' })
  @ApiResponse({ status: 404, description: 'Project or collaborator not found' })
  async revokeGuest(
    @Param('projectId') projectId: string,
    @Param('collaboratorId') collaboratorId: string,
    ...
    return this.collaboratorsService.revokeGuestAccess(projectId, collaboratorId, req.user.id);
  }

  @Post('project/:projectId/:collaboratorId/regenerate')
  @ApiOperation({ summary: 'Regenerate guest invite link' })
  @ApiResponse({ status: 200, description: 'Invite link regenerated successfully' })
  @ApiResponse({ status: 400, description: 'Cannot regenerate link for internal users' })
  @ApiResponse({ status: 403, description: 'Permission denied (must be OWNER)' })
  @ApiResponse({ status: 404, description: 'Project or collaborator not found' })
  async regenerateInvite(
    ...
    return this.collaboratorsService.regenerateGuestInvite(projectId, collaboratorId, req.user.id);
  }
}
