...
export class MediaFoldersController {
  ...
  @ApiResponse({ status: 404, description: 'Project or parent folder not found' })
  async createFolder(@Request() req: AuthenticatedRequest, @Body() createFolderDto: CreateFolderDto) {
  ...
  async getFolderTree(@Request() req: AuthenticatedRequest, @Param('projectId') projectId: string) {
  ...
  async getFolderContents(@Request() req: AuthenticatedRequest, @Param('folderId') folderId: string) {
  ...
  @ApiParam({ name: 'folderId', description: 'Folder UUID' })
  @ApiResponse({ status: 200, description: 'Folder path retrieved successfully' })
  @ApiResponse({ status: 404, description: 'Folder not found' })
  async getFolderPath(@Request() req: AuthenticatedRequest, @Param('folderId') folderId: string) {
  ...
  @ApiResponse({ status: 400, description: 'Bad request (duplicate name or circular reference)' })
  @ApiResponse({ status: 404, description: 'Folder not found' })
  async updateFolder(
  ...
  @Delete(':folderId')
  @ApiOperation({
    summary: 'Delete folder (cascade deletes subfolders, assets from DB, and R2 storage files)',
    description: 'Permanently deletes folder and all contents: nested folders, asset database records, and actual files from R2 storage'
  })
  @ApiParam({ name: 'folderId', description: 'Folder UUID' })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({ status: 404, description: 'Folder not found' })
  @ApiResponse({ status: 403, description: 'Access denied (requires OWNER or EDITOR role)' })
  async deleteFolder(@Request() req: AuthenticatedRequest, @Param('folderId') folderId: string) {
  ...
  async moveAssets(
    ...
    return this.foldersService.moveAssets(req.user.id, projectId, moveAssetsDto);
  }
}
