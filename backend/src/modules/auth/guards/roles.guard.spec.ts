...
describe('RolesGuard', () => {
  ...
  describe('Basic Functionality', () => {
    ...
    it('should allow access when no roles are required', () => {
      const mockContext = createMockExecutionContext({ id: '1', role: UserRole.STAFF });
  ...
  describe('Error Message Tests', () => {
    it('should include required roles in error message', () => {
      ...
      expect(() => guard.canActivate(mockContext)).toThrow(/Super Admin, Finance Manager/);
    });
    ...
    it('should include current role in error message', () => {
      ...
      expect(() => guard.canActivate(mockContext)).toThrow(/Your current role: Staff/);
    });
  });
  ...
  describe('Real-World Scenario Tests', () => {
    ...
    it('should deny ACCOUNTANT from approving quotations', () => {
      ...
      expect(() => guard.canActivate(mockContext)).toThrow(ForbiddenException);
      expect(() => guard.canActivate(mockContext)).toThrow(/Super Admin, Finance Manager/);
      expect(() => guard.canActivate(mockContext)).toThrow(/Your current role: Accountant/);
    });
    ...
    it('should deny PROJECT_MANAGER from managing users', () => {
      ...
      expect(() => guard.canActivate(mockContext)).toThrow(ForbiddenException);
      expect(() => guard.canActivate(mockContext)).toThrow(/Super Admin/);
      expect(() => guard.canActivate(mockContext)).toThrow(/Your current role: Project Manager/);
    });
  });
});
