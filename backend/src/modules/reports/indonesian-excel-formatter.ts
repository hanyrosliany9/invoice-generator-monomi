...
export interface IndonesianReportHeader {
  ...
  reportType: 'LAPORAN_POSISI_KEUANGAN' | 'LAPORAN_LABA_RUGI' | 'LAPORAN_ARUS_KAS' | 'RINCIAN_PENJUALAN' | 'RINCIAN_PIUTANG';
}
...
export class IndonesianExcelFormatter {
  ...
  static formatIndonesianLetterhead(
    ...
    if (companyInfo.website || companyInfo.npwp) {
      ...
      if (companyInfo.website) additionalInfo.push(`Website: ${companyInfo.website}`);
      if (companyInfo.npwp) additionalInfo.push(`NPWP: ${companyInfo.npwp}`);
      
      const additionalRow = worksheet.addRow([additionalInfo.join(' | ')]);
      additionalRow.font = { name: 'Arial', size: 9, color: { argb: 'FF666666' } };
    ...
    if (reportHeader.reportSubtitle) {
      const subtitleRow = worksheet.addRow([reportHeader.reportSubtitle.toUpperCase()]);
      subtitleRow.font = { 
        name: 'Arial', 
        size: 12, 
        bold: true, 
   ...
   */
  static applyIndonesianCurrencyFormat(worksheet: ExcelJS.Worksheet, columns: number[]): void {
   ...
   */
  static applyIndonesianDateFormat(worksheet: ExcelJS.Worksheet, columns: number[]): void {
    columns.forEach(colNum => {
      const column = worksheet.getColumn(colNum);
   ...
   */
  static formatNegativeNumbers(worksheet: ExcelJS.Worksheet, columns: number[]): void {
    columns.forEach(colNum => {
      const column = worksheet.getColumn(colNum);
   ...
   */
  static setIndonesianColumnWidths(worksheet: ExcelJS.Worksheet, columnCount: number): void {
   ...
   */
  static validateIndonesianCompliance(companyInfo: IndonesianCompanyInfo): string[] {
    const errors: string[] = [];
    ...
    if (companyInfo.phone && !companyInfo.phone.match(/^(\+62|62|0)[0-9]{8,11}$/)) {
      errors.push('Format nomor telepon Indonesia tidak valid');
    }
    
    // Validate NPWP format if provided
    if (companyInfo.npwp && !companyInfo.npwp.match(/^[0-9]{2}\.[0-9]{3}\.[0-9]{3}\.[0-9]{1}-[0-9]{3}\.[0-9]{3}$/)) {
