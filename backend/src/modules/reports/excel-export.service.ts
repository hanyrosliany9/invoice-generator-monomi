...
export class ExcelExportService {
  ...
  async generateSalesAndReceivablesReport(
    ...
    this.createSalesClientSummarySheet(
      workbook,
      salesClientSummary,
      companyInfo,
      targetMonth,
    );
    this.createReceivablesMonthlySummarySheet(
      workbook,
      receivablesMonthlySummary,
      companyInfo,
      targetMonth,
  ...
  private async getSalesClientSummary(
    filters: ExportFilters,
  ): Promise<ClientMonthlySummary[]> {
  ...
  private async getReceivablesClientSummary(
    filters: ExportFilters,
  ): Promise<ClientMonthlySummary[]> {
    return this.getReceivablesMonthlySummary(filters);
  }

  private createSalesDetailSheet(
    ...
    const tableEndRow =
      materaiCount > 0 ? summaryRowIndex + 2 : summaryRowIndex;
  ...
  private createReceivablesDetailSheet(
    workbook: ExcelJS.Workbook,
    ...
    const startCol = 1;
    const endCol = 9;
    ExcelTableHelper.formatAsTable(
      worksheet,
      startRow,
      endRow,
      startCol,
  ...
  private createSalesMonthlySummarySheet(
    workbook: ExcelJS.Workbook,
  ...
  private createSalesClientSummarySheet(
    workbook: ExcelJS.Workbook,
    data: ClientMonthlySummary[],
    companyInfo: IndonesianCompanyInfo,
    targetMonth?: number | null,
    ...
    if (targetMonth !== null && targetMonth !== undefined) {
      ...
      const total = data.reduce(
        (sum, item) => sum + ((item[targetMonthName] as number) || 0),
        0,
      );
      worksheet.addRow(["JUMLAH TOTAL", total]);

      // Apply table formatting for single month (2 columns)
      ExcelTableHelper.formatClientSummaryTable(
        worksheet,
        5,
        data.length,
        2,
    ...
    worksheet.columns.forEach((column: any) => {
      column.width =
        targetMonth !== null && targetMonth !== undefined ? 18 : 12;
    });
  }

  private createReceivablesMonthlySummarySheet(
    workbook: ExcelJS.Workbook,
    data: ClientMonthlySummary[],
    companyInfo: IndonesianCompanyInfo,
    targetMonth?: number | null,
  ...
  private createReceivablesClientSummarySheet(
    workbook: ExcelJS.Workbook,
    data: ClientMonthlySummary[],
    companyInfo: IndonesianCompanyInfo,
    targetMonth?: number | null,
    ...
    if (targetMonth !== null && targetMonth !== undefined) {
      ...
      data.forEach((item) => {
        const clientPenjualan = monthNames.reduce(
  ...
  private validateIndonesianBalance(
    saldoAwal: number,
    penjualan: number,
    pembayaran: number,
    saldoAkhir: number,
