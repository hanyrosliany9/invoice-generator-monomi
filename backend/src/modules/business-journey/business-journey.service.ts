...
export class BusinessJourneyService {
  ...
  async createEvent(
    ...
    try {
      ...
      if (!compliance.isCompliant) {
        this.logger.warn(`Materai compliance issues: ${compliance.warnings.join(', ')}`)
      }
      ...
      const event = await this.prisma.businessJourneyEvent.create({
        data: {
          ...
          amount: createEventDto.amount ? new Prisma.Decimal(createEventDto.amount) : null,
          ...
          metadata: createEventDto.metadata ? {
            create: {
              ...
              source: createEventDto.metadata.source || BusinessJourneyEventSource.USER,
              priority: createEventDto.metadata.priority || BusinessJourneyPriority.MEDIUM,
              tags: createEventDto.metadata.tags || [],
              relatedDocuments: createEventDto.metadata.relatedDocuments || [],
              ...
              materaiRequired: createEventDto.metadata.materaiRequired || false,
              materaiAmount: createEventDto.metadata.materaiAmount ? 
                new Prisma.Decimal(createEventDto.metadata.materaiAmount) : null,
    ...
    try {
      ...
      const updatedEvent = await this.prisma.businessJourneyEvent.update({
        ...
        data: {
          ...
          amount: updateEventDto.amount ? new Prisma.Decimal(updateEventDto.amount) : undefined,
          metadata: updateEventDto.metadata ? {
            update: {
              ...
              materaiAmount: updateEventDto.metadata.materaiAmount ? 
                new Prisma.Decimal(updateEventDto.metadata.materaiAmount) : undefined,
  ...
  async autoCreateEvent(
    ...
    try {
      const eventType = this.mapEntityActionToEventType(entityType, action, metadata)
      ...
      const createEventDto: CreateBusinessJourneyEventDto = {
        ...
        description: this.generateEventDescription(eventType, entityDetails, metadata),
  ...
  private buildWhereConditions(clientId: string, filters: BusinessJourneyFiltersDto): Prisma.BusinessJourneyEventWhereInput {
  ...
  private async getBusinessJourneySummary(clientId: string): Promise<BusinessJourneySummaryDto> {
  ...
  private async getMateraiComplianceStatus(clientId: string): Promise<MateraiComplianceStatusDto> {
    ...
    for (const invoice of invoices) {
      ...
      if (amount >= 5000000) { // 5 million IDR threshold
    ...
    const compliancePercentage = totalRequiredAmount > 0 ? (appliedAmount / totalRequiredAmount) * 100 : 100
  ...
  private validateMateraiCompliance(createEventDto: CreateBusinessJourneyEventDto): ComplianceResult {
  ...
  private generateEventTitle(eventType: BusinessJourneyEventType, entityDetails: any): string {
  ...
  private generateEventDescription(eventType: BusinessJourneyEventType, entityDetails: any, metadata?: any): string {
  ...
  private getInitialEventStatus(eventType: BusinessJourneyEventType): BusinessJourneyEventStatus {
    const statusMap: Record<BusinessJourneyEventType, BusinessJourneyEventStatus> = {
      [BusinessJourneyEventType.CLIENT_CREATED]: BusinessJourneyEventStatus.COMPLETED,
      [BusinessJourneyEventType.PROJECT_STARTED]: BusinessJourneyEventStatus.IN_PROGRESS,
      [BusinessJourneyEventType.QUOTATION_DRAFT]: BusinessJourneyEventStatus.PENDING,
      [BusinessJourneyEventType.QUOTATION_SENT]: BusinessJourneyEventStatus.IN_PROGRESS,
      [BusinessJourneyEventType.QUOTATION_APPROVED]: BusinessJourneyEventStatus.COMPLETED,
      [BusinessJourneyEventType.QUOTATION_DECLINED]: BusinessJourneyEventStatus.CANCELLED,
      [BusinessJourneyEventType.QUOTATION_REVISED]: BusinessJourneyEventStatus.IN_PROGRESS,
      [BusinessJourneyEventType.INVOICE_GENERATED]: BusinessJourneyEventStatus.COMPLETED,
      [BusinessJourneyEventType.INVOICE_SENT]: BusinessJourneyEventStatus.IN_PROGRESS,
      [BusinessJourneyEventType.PAYMENT_RECEIVED]: BusinessJourneyEventStatus.COMPLETED,
      [BusinessJourneyEventType.PAYMENT_OVERDUE]: BusinessJourneyEventStatus.REQUIRES_ATTENTION,
      [BusinessJourneyEventType.MATERAI_REQUIRED]: BusinessJourneyEventStatus.REQUIRES_ATTENTION,
      [BusinessJourneyEventType.MATERAI_APPLIED]: BusinessJourneyEventStatus.COMPLETED
    }

    return statusMap[eventType] || BusinessJourneyEventStatus.PENDING
  }

  private getEventPriority(eventType: BusinessJourneyEventType): BusinessJourneyPriority {
    const priorityMap: Record<BusinessJourneyEventType, BusinessJourneyPriority> = {
      ...
      [BusinessJourneyEventType.QUOTATION_DRAFT]: BusinessJourneyPriority.MEDIUM,
      [BusinessJourneyEventType.QUOTATION_SENT]: BusinessJourneyPriority.HIGH,
      [BusinessJourneyEventType.QUOTATION_APPROVED]: BusinessJourneyPriority.HIGH,
      [BusinessJourneyEventType.QUOTATION_DECLINED]: BusinessJourneyPriority.MEDIUM,
      [BusinessJourneyEventType.QUOTATION_REVISED]: BusinessJourneyPriority.HIGH,
      [BusinessJourneyEventType.INVOICE_GENERATED]: BusinessJourneyPriority.HIGH,
      ...
      [BusinessJourneyEventType.PAYMENT_OVERDUE]: BusinessJourneyPriority.CRITICAL,
      [BusinessJourneyEventType.MATERAI_REQUIRED]: BusinessJourneyPriority.HIGH,
      [BusinessJourneyEventType.MATERAI_APPLIED]: BusinessJourneyPriority.MEDIUM
    }

    return priorityMap[eventType] || BusinessJourneyPriority.MEDIUM
  }

  private generateEventTags(eventType: BusinessJourneyEventType, entityDetails: any): string[] {
  ...
  private mapEventToResponse(event: any): BusinessJourneyEventResponseDto {
    return {
      ...
      metadata: event.metadata ? {
        ...
        materaiAmount: event.metadata.materaiAmount ? Number(event.metadata.materaiAmount) : undefined,
