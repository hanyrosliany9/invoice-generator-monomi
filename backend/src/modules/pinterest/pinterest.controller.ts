...
import { PinterestPinListResponseDto } from "./dto/pin.dto";
...
export class PinterestController {
  ...
  @ApiResponse({
    ...
    type: PinterestJobListResponseDto,
  })
  async getJobs(
    @Req() req: any,
  ...
  @ApiResponse({
    ...
    type: PinterestPinListResponseDto,
  })
  @ApiResponse({ status: 404, description: "Job not found" })
  async getPins(
  ...
  @ApiResponse({
    status: 200,
    description: "ZIP file containing all downloaded pins",
  })
  @ApiResponse({
    status: 404,
    description: "Job not found or no files available",
  })
  async downloadJobZip(
    ...
    const { stream: archive, filename } =
      await this.pinterestService.createJobZip(id, req.user.id);
  ...
  async getJobPath(
    ...
    const outputPath = await this.pinterestService.getJobOutputPath(
      id,
      req.user.id,
    );
  ...
  async getPinInfo(@Body("url") url: string): Promise<{
    pinId: string;
    title?: string;
    description?: string;
