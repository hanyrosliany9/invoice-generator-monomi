-- Every ADD CONSTRAINT below is wrapped in a DO block that swallows
-- duplicate_object: this migration was generated against a dev database
-- whose FK constraints had drifted (gone missing) relative to the
-- baseline migration's history. Some of these constraints already exist
-- from baseline; replaying this migration from scratch against a
-- freshly-baselined database would otherwise fail on the first one
-- (refresh_tokens_userId_fkey) with "constraint already exists" and
-- abort the whole migration, including the genuinely-new constraints
-- later in this file. Wrapping every statement makes this migration
-- safe to replay from any starting state.
